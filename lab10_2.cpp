...
int drawCard(void){
	//TODO Write the function to random the number from 1 to 13 and return that random number.
	return rand()%13+1;
}
...
int findYugiAction(int s){	
	if(s == 9) return 2; //? Yugi will definitely stay (2) when current score (s) is equal to 9
		...
		int YugiRand = rand()%100 + 1;
		if(YugiRand<70) return 1;
		else return 2; 
...
void checkWinner(int p, int y){
	//TODO Write condition for cout in this function
	cout << "\n---------------------------------\n";
...
int main(){	
	...
	if(playerAction == 1){  //! Destiny Draw
